    ...
    <div>
      ...
      <div className=" bg-amber-50 px-4 py-28 md:px-20 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {blogList.map((blog) => (
          <div
            key={blog.id}
            className="bg-white p-3  shadow hover:shadow-lg transition-all duration-300 transform hover:-translate-y-3"
          >
            <img
              src={blog.imgUrl}
              alt={blog.imgAlt}
            ...
            <div className="p-5 space-y-3">
              <Link
                  to={`/blog/${blog.id}`}
               className="text-lg font-bold leading-snug text-gray-800 hover:text-orange-500 cursor-pointer">
                {blog.title}
              </Link>
